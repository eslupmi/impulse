...
class GoogleCalendarChain(ScheduleChain):
    ...
    def _make_api_request(self, method: str, url: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(e.response, 'text'):
                logger.error(f"Response text: {e.response.text}", extra={'provider': 'google'})
            raise
    ...
    def _update_timezone(self, new_timezone: str) -> None:
        ...
        if new_timezone != self.timezone:
            logger.info(f"Updating timezone from {self.timezone} to {new_timezone}", extra={'provider': 'google'})
    ...
    def _fetch_initial_data(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error during initial calendar sync: {str(e)}", extra={'provider': 'google'})
        ...
        if self._sync_task is None:
            try:
                ...
                if loop.is_running():
                    ...
                    logger.info(f"Started calendar sync task", extra={'provider': 'google'})
                else:
                    logger.warning(f"Event loop not running, cannot start calendar sync task", extra={'provider': 'google'})
            except RuntimeError:
                logger.error(f"Failed to start calendar sync task, no event loop", extra={'provider': 'google'})
    ...
    def stop_sync(self) -> None:
        ...
        if self._sync_task is not None:
            ...
            self._sync_task = None
            logger.info(f"Stopped calendar sync task", extra={'provider': 'google'})
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            ...
            logger.error(f"JWT token generation failed: {str(e)}", extra={'provider': 'google'})
            raise
        except requests.exceptions.RequestException as e:
            ...
            logger.error(f"Missing key in response: {str(e)}", extra={'provider': 'google'})
            raise
    ...
    def _get_calendar_timezone(self) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to get calendar timezone: {str(e)}", extra={'provider': 'google'})
    ...
    def _fetch_events(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to fetch calendar events: {str(e)}", extra={'provider': 'google'})
            return []
    ...
    def _parse_steps_from_description(description: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.warning(f"Failed to parse description due to invalid input: {str(e)}", extra={'provider': 'google'})
        except MemoryError as e:
            logger.error(f"Memory error while parsing description (content too large): {str(e)}", extra={'provider': 'google'})
        except Exception as e:
            logger.warning(f"Unexpected error while parsing description: {str(e)}", extra={'provider': 'google'})
    ...
    async def _sync_calendar(self) -> None:
        ...
        while True:
            try:
                ...
                events = self._fetch_events()
                self._update_schedule(events)
                logger.debug(f"Synced {len(events)} events from Google Calendar", extra={'provider': 'google'})

            except Exception as e:
                logger.error(f"Error syncing сalendar: {str(e)}", extra={'provider': 'google'})
