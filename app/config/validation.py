...
class TelegramChannel(BaseUser):
    ...
    id: Union[int, str] = Field(..., description="Channel ID or fixed topic in format '<chat_id>_<topic_id>'")
    name: Optional[str] = Field(None, description="Channel name")

    @field_validator('id', mode='before')
    @classmethod
    def validate_id_format(cls, v):
        """Allow int chat_id or string in '<chat_id>_<topic_id>' or '<chat_id>/<topic_id>' formats."""
        if isinstance(v, int):
            return v
        if isinstance(v, str):
            s = v.strip()
            ...
            if re.match(r'^-?\d+(?:[\/_]\d+)?$', s):
                return s
        raise ValueError("Telegram channel id must be int chat_id or string '<chat_id>_<topic_id>' or '<chat_id>/<topic_id>'")
...
class IncidentNotifications(BaseModel):
    ...
    resolved: Optional[bool] = Field(False, description="Resolved notification (compat: alerts_resolved_notifications)")
...
class ExternalJWTConfig(BaseModel):
    env_var_name: Optional[str] = Field('GRAFANA_JWT_TOKEN', description='Env var with JWT token')
    http_url: Optional[str] = Field(None, description='HTTP URL to fetch JWT token')
    http_method: Optional[str] = Field('GET', description='HTTP method')
    http_headers: Optional[Dict[str, Any]] = Field(None, description='HTTP headers JSON')
    http_body: Optional[Dict[str, Any]] = Field(None, description='HTTP body JSON')
    http_token_json_path: Optional[str] = Field('access_token', description='JSON path to token')
    http_cache_ttl_seconds: Optional[int] = Field(240, description='Cache TTL seconds')
    http_timeout_seconds: Optional[int] = Field(10, description='HTTP timeout seconds')
    http_retries: Optional[int] = Field(2, description='HTTP retries')
    http_retry_backoff_ms: Optional[int] = Field(300, description='HTTP retry backoff ms')
    clock_skew_seconds: Optional[int] = Field(15, description='Clock skew seconds')
    allow_fallback_to_disabled: Optional[bool] = Field(False, description='Allow fallback to disabled')


class JWTAuthConfig(BaseModel):
    mode: Optional[str] = Field('internal', description='JWT auth mode: disabled|internal|external_env|external_http')
    enabled: Optional[bool] = Field(False, description='Enable JWT auth')
    keys_dir: Optional[str] = Field(None, description='Keys directory')
    kid: Optional[str] = Field('impulse-key-1', description='Key ID')
    issuer: Optional[str] = Field('http://localhost:5000', description='JWT issuer')
    audience: Optional[str] = Field('impulse', description='JWT audience')
    ttl_seconds: Optional[int] = Field(300, description='JWT TTL seconds')
    rotation_enabled: Optional[bool] = Field(False, description='Enable key rotation')
    rotation_interval_days: Optional[int] = Field(120, description='Rotation interval days')
    grace_period_seconds: Optional[int] = Field(900, description='Grace period seconds')
    max_keys: Optional[int] = Field(3, description='Max keys stored')
    external_jwt: Optional[ExternalJWTConfig] = Field(None, description='External JWT provider config')


class PanelVariablesConfig(BaseModel):
    """Grafana panel variables mapping configuration"""
    default_mapping: Optional[Dict[str, str]] = Field(default_factory=dict, description='Default label->var mapping')
    dashboard_specific: Optional[Dict[str, Dict[str, str]]] = Field(default_factory=dict, description='Per-dashboard mapping overrides')


class GrafanaRendererConfig(BaseModel):
    renderer_url: Optional[str] = Field('http://renderer:8081', description='Renderer URL')
    grafana_url: Optional[str] = Field('http://grafana:3000', description='Grafana URL')
    render_key: Optional[str] = Field(None, description='Grafana render key')
    enabled: Optional[bool] = Field(False, description='Enable renderer')
    rate_limit: Optional[int] = Field(60, description='Rate limit seconds')
    max_size: Optional[int] = Field(10 * 1024 * 1024, description='Max image size bytes')
    time_to_render: Optional[int] = Field(15, description='Time to render minutes')
    jwt_auth: Optional[JWTAuthConfig] = Field(None, description='JWT auth config')
    panel_variables_max_values_per_var: Optional[int] = Field(20, description='Panel variables max values per var')
    panel_variables_max_url_length: Optional[int] = Field(8192, description='Panel variables max url length')
    panel_variables: Optional[PanelVariablesConfig] = Field(default_factory=PanelVariablesConfig, description='Panel variables mapping config')
...
class ImpulseConfig(BaseModel):
    ...
    grafana_renderer: Optional['GrafanaRendererConfig'] = Field(None, description="Grafana renderer configuration")
...
IncidentNotifications.model_rebuild()
