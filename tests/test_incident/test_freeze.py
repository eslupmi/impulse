...
class TestIncidentInhibitionFreeze:
    ...
    def test_unfreeze_from_time_based_sets_chain_enabled_false(self, sample_incident):
        """Test that unfreezing from time-based freeze sets chain_enabled to False."""
        sample_incident.chain_enabled = True
        freeze_until = datetime.now(timezone.utc) + timedelta(hours=1)
        
        with patch.object(sample_incident, 'dump'):
            sample_incident.freeze(freeze_until, "U123456")
            assert sample_incident.chain_enabled is False  # time-based freeze disables
            
            sample_incident.unfreeze()
            
            # chain_enabled should be False (time-based unfreeze sets it to False)
            assert sample_incident.chain_enabled is False
    ...
    def test_is_frozen_with_time_only(self, sample_incident):
        """Test is_frozen returns True when only frozen_until is set."""
        freeze_until = datetime.now(timezone.utc) + timedelta(hours=1)
        with patch.object(sample_incident, 'dump'):
            sample_incident.freeze(freeze_until, "U123456")
        
        assert sample_incident.frozen_until is not None
        assert sample_incident.frozen_by_inhibition is False
        assert sample_incident.is_frozen() is True

    def test_is_frozen_with_both(self, sample_incident):
        """Test is_frozen returns True when both freeze types are active."""
        freeze_until = datetime.now(timezone.utc) + timedelta(hours=1)
        with patch.object(sample_incident, 'dump'):
            sample_incident.freeze(freeze_until, "U123456")
