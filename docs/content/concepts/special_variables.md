...
- `childs` - map of uniq_ids to `incident` objects this incident inhibits (see [inhibition](inhibition.md))
- `env`[↰](../config_file.md#webhooks) - to access environment variables (see [example](../integrations/external/telegram.md))
...
- `incidents` - all incidents (`uniq_ids` map). Deprecated, use `parents` / `childs` instead
- `parents` - map of uniq_ids to incident objects that inhibit this incident (see [inhibition](inhibition.md))
