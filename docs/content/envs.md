...
| HTTP_PROXY | HTTP proxy URL for outbound HTTP requests<br/>(standard; also `http_proxy`) | | - |
| HTTPS_PROXY | HTTPS proxy URL for outbound HTTPS requests<br/>(standard; also `https_proxy`) | | - |
...
| NO_PROXY | Comma-separated hosts that bypass the proxy<br/>(standard; also `no_proxy`; e.g. `127.0.0.1,localhost`) | | - |
